from fastapi.testclient import TestClient
from src.main import app
from tests.conftest import sample_user_json, sample_session_json
from tests.utils.utils_test import (delete_all_user_items, 
                                    delete_all_session_items, 
                                    create_expired_token)

client = TestClient(app)

def test_login_invalid(sample_session_json, sample_user_json):

    delete_all_user_items()

    response = client.post("/v1/users/auth/register", json=sample_user_json)
    assert response.status_code == 201

    # Send POST request to login the user
    response = client.post("/v1/users/auth/login", json=sample_session_json)
    assert response.status_code == 201

    response = client.get("/v1/users/auth/login/validate", 
                          params={"JWT": "wrong"})
    assert response.status_code == 401
    assert response.json() == {"detail": "Invalid token"}

def test_login_expired(sample_session_json, sample_user_json):

    delete_all_user_items()

    response = client.post("/v1/users/auth/register", json=sample_user_json)
    assert response.status_code == 201
    
    token = create_expired_token()

    response = client.get("/v1/users/auth/login/validate", 
                          params={"JWT": token})
    assert response.status_code == 401
    assert response.json() == {"detail": "Token expired"}

def test_login_missing(sample_session_json, sample_user_json):

    delete_all_user_items()

    response = client.post("/v1/users/auth/register", json=sample_user_json)
    assert response.status_code == 201

    # Send POST request to login the user
    response = client.post("/v1/users/auth/login", json=sample_session_json)
    assert response.status_code == 201

    response = client.get("/v1/users/auth/login/validate", 
                          params={ "JWT" : "" })
    assert response.status_code == 401
    assert response.json() == {"detail": "Token missing"}

def test_login_user_success(sample_session_json, sample_user_json):

    delete_all_user_items()
    delete_all_session_items()
    response = client.post("/v1/users/auth/register", json=sample_user_json)

    # Send POST request to login the user
    token = client.post("/v1/users/auth/login", json=sample_session_json)
    assert token.status_code == 201
    assert "JWT" in token.json()

    response = client.get("/v1/users/auth/login/validate", 
                          params={"JWT": token.json()["JWT"]})
    assert response.status_code == 201
    assert response.json() == {"valid": True}
