...
class OrderParser:
    """
    A class to parse UBL order documents into an enriched OrderType.
    """
    ...
    def get_text(element, path, required=False, error_message=None, ns=None):
        """
        Retrieve text content from an XML element.

        :param element: The XML element to search within.
        :param path: The XPath to the desired child element.
        :param required: Whether the text content is required.
        :param error_message: Custom error message if required text is missing.
    ...
    def format_address(address_elem, ns=None) -> str:
        """
        Format an address element into a string.

        :param address_elem: The XML element containing the address.
        :param ns: XML namespaces.
        ...
        parts = [
            ...
            OrderParser.get_text(
                address_elem, "./cac:Country/cbc:IdentificationCode", ns=ns
            )
            or "",
        ]
    ...
    def parse_order_header(root, ns):
        """
        Parse the order header from the XML root element.

        :param root: The XML root element.
        :param ns: XML namespaces.
        :return: A dictionary containing order header details.
        """
        order_id = OrderParser.get_text(
            root,
            "./cbc:ID",
            required=True,
            error_message="Missing order ID in XML",
            ns=ns,
        )
        ...
        issue_date_str = OrderParser.get_text(
            ...
            "./cbc:IssueDate",
            required=True,
            error_message="Missing IssueDate in XML",
            ns=ns,
        )
    ...
    def parse_buyer_details(root, ns):
        """
        Parse buyer details from the XML root element.

        :param root: The XML root element.
        :param ns: XML namespaces.
        :return: A dictionary containing buyer details.
        """
        buyer_customer = root.find("./cac:BuyerCustomerParty", ns)
        if buyer_customer is None:
            raise HTTPException(
                status_code=400, detail="Missing BuyerCustomerParty in XML"
            )
        buyer_name = OrderParser.get_text(
            buyer_customer,
            ".//cac:PartyName/cbc:Name",
            required=True,
            error_message="Missing buyer name in XML",
            ns=ns,
        )
        buyer_account = OrderParser.get_text(
            ...
            required=True,
            error_message="Missing buyer account in XML",
            ns=ns,
        )
        ...
        buyer_electronic_address = OrderParser.get_text(
            buyer_customer, "./cac:Party/cbc:EndpointID", ns=ns
        )
        ...
        buyer_country = (
            OrderParser.get_text(
                buyer_address_elem, "./cac:Country/cbc:IdentificationCode", ns=ns
            )
            if buyer_address_elem is not None
            else None
        )
        return {
    ...
    def parse_seller_details(root, ns):
        """
        Parse seller details from the XML root element.

        :param root: The XML root element.
        :param ns: XML namespaces.
        :return: A dictionary containing seller details.
        """
        seller_supplier = root.find("./cac:SellerSupplierParty", ns)
        if seller_supplier is None:
            raise HTTPException(
                status_code=400, detail="Missing SellerSupplierParty in XML"
            )
        seller_account = OrderParser.get_text(
            seller_supplier,
            "./cbc:CustomerAssignedAccountID",
            required=True,
            error_message="Missing seller account in XML",
            ns=ns,
        )
        seller_name = OrderParser.get_text(
            ...
            ".//cac:PartyName/cbc:Name",
            required=True,
            error_message="Missing seller name in XML",
            ns=ns,
        )
    ...
    def parse_monetary_totals(root, ns):
        """
        Parse monetary totals from the XML root element.

        :param root: The XML root element.
        :param ns: XML namespaces.
        :return: A dictionary containing monetary totals.
        """
        anticipated_total = root.find("./cac:AnticipatedMonetaryTotal", ns)
        if anticipated_total is None:
            raise HTTPException(
                status_code=400, detail="Missing AnticipatedMonetaryTotal in XML"
            )
        line_ext_str = OrderParser.get_text(
            anticipated_total,
            "./cbc:LineExtensionAmount",
            required=True,
            error_message="Missing LineExtensionAmount in XML",
            ns=ns,
        )
        payable_str = OrderParser.get_text(
            ...
            "./cbc:PayableAmount",
            required=True,
            error_message="Missing PayableAmount in XML",
            ns=ns,
        )
        return {
            "anticipated_line_extension_amount": OrderParser.parse_float(
                line_ext_str, "LineExtensionAmount"
            ),
            "anticipated_payable_amount": OrderParser.parse_float(
                payable_str, "PayableAmount"
            ),
        }
    ...
    def parse_payment_terms(root, ns):
        """
        Parse payment terms from the XML root element.

        :param root: The XML root element.
        :param ns: XML namespaces.
        :return: A string containing payment terms.
        """
        transaction_conditions = root.find("./cac:TransactionConditions", ns)
        return (
            OrderParser.get_text(transaction_conditions, "./cbc:Description", ns=ns)
    ...
    def parse_order_line(order_line_elem, ns):
        """
        Parse an individual order line from the XML element.

        :param order_line_elem: The XML element containing the order line.
        :param ns: XML namespaces.
        :return: An OrderLineType object.
        """
        ...
        line_extension_str = OrderParser.get_text(
            line_item_elem, "./cbc:LineExtensionAmount", ns=ns
        )
        tax_amount_str = OrderParser.get_text(
            line_item_elem, "./cbc:TotalTaxAmount", ns=ns
        )
        price_elem = line_item_elem.find("./cac:Price", ns)
        unit_price_str = (
            OrderParser.get_text(price_elem, "./cbc:PriceAmount", ns=ns)
        ...
        if item_elem is None:
            return None
        item_description = (
            OrderParser.get_text(item_elem, "./cbc:Description", ns=ns) or ""
        )
        item_name = OrderParser.get_text(item_elem, "./cbc:Name", ns=ns) or ""
        buyers_item_id = (
            OrderParser.get_text(
                item_elem, "./cac:BuyersItemIdentification/cbc:ID", ns=ns
            )
            or ""
        )
        sellers_item_id = (
            OrderParser.get_text(
                item_elem, "./cac:SellersItemIdentification/cbc:ID", ns=ns
            )
            or ""
        )

        quantity = OrderParser.parse_float(quantity_str, "Quantity")
        line_extension_amount = OrderParser.parse_float(
            line_extension_str, "LineExtensionAmount"
        )
        total_tax_amount = OrderParser.parse_float(tax_amount_str, "TotalTaxAmount")
        unit_price = (
            OrderParser.parse_float(unit_price_str, "PriceAmount")
            if unit_price_str
            else 0.0
        )
    ...
    def parse_order_lines(root, ns):
        """
        Parse all order lines from the XML root element.

        :param root: The XML root element.
        :param ns: XML namespaces.
        :return: A list of OrderLineType objects.
        """
        return [
            line
            for line in (
    ...
    def parse_xml_order(content: bytes) -> OrderType:
        """
        Parse an XML order document into an OrderType object.

        :param content: The XML content as bytes.
        ...
        return OrderType(
            ...
            anticipated_line_extension_amount=monetary[
                "anticipated_line_extension_amount"
            ],
