...
from src.models.common.tax_scheme import TaxScheme, PartyTaxScheme
...
class OrderXmlExtractor:
    ...
    def format_address(address_elem, ns=None) -> str:
        ...
        parts = [
            ...
            OrderXmlExtractor.get_text(address_elem, "./cbc:BuildingNumber", ns=ns)
            or "",
            ...
            OrderXmlExtractor.get_text(address_elem, "./cbc:CountrySubentity", ns=ns)
            or "",
            OrderXmlExtractor.get_text(address_elem, "./cac:AddressLine", ns=ns) or "",
            OrderXmlExtractor.get_text(
    ...
    def extract_contact(contact_elem, ns=None):
        ...
        ns = ns or OrderXmlExtractor.NAMESPACES
        return Contact(
            name=OrderXmlExtractor.get_text(
                contact_elem, "./cbc:Name", required=True, ns=ns
            ),
            telephone=OrderXmlExtractor.get_text(
                contact_elem, "./cbc:Telephone", ns=ns
            ),
            telefax=OrderXmlExtractor.get_text(contact_elem, "./cbc:Telefax", ns=ns),
            electronic_mail=OrderXmlExtractor.get_text(
                contact_elem, "./cbc:ElectronicMail", ns=ns
            ),
        )
    ...
    def extract_tax_scheme(tax_scheme_elem, ns=None):
        ...
        ns = ns or OrderXmlExtractor.NAMESPACES
        return TaxScheme(
            id=OrderXmlExtractor.get_text(
                tax_scheme_elem, "./cbc:ID", required=True, ns=ns
            ),
            tax_type_code=OrderXmlExtractor.get_text(
                tax_scheme_elem, "./cbc:TaxTypeCode", ns=ns
            ),
        )
    ...
    def extract_party_tax_scheme(party_tax_scheme_elem, ns=None):
        ...
        return PartyTaxScheme(
            registration_name=OrderXmlExtractor.get_text(
                party_tax_scheme_elem, "./cbc:RegistrationName", ns=ns
            ),
            company_id=OrderXmlExtractor.get_text(
                party_tax_scheme_elem, "./cbc:CompanyID", required=True, ns=ns
            ),
            exemption_reason=OrderXmlExtractor.get_text(
    ...
    def extract(xml_content: bytes) -> dict:
        ...
        header = {
            "order_reference": OrderXmlExtractor.get_text(
                root,
                "./cbc:ID",
                required=True,
            ...
            "buyer_reference": OrderXmlExtractor.get_text(
                root, "./cbc:SalesOrderID", ns=ns
            )
            or "",
        ...
        buyer = {
            "buyer_name": OrderXmlExtractor.get_text(
                buyer_party,
            ...
            "buyer_account_customer_id": OrderXmlExtractor.get_text(
                buyer_customer,
                "./cbc:CustomerAssignedAccountID",
                required=True,
                error_message="Missing buyer customer account in XML",
                ns=ns,
            ),
            "buyer_account_supplier_id": OrderXmlExtractor.get_text(
                buyer_customer,
                "./cbc:SupplierAssignedAccountID",
                required=True,
                error_message="Missing buyer supplier account in XML",
                ns=ns,
            ),
            "buyer_address": OrderXmlExtractor.format_address(buyer_address_elem, ns),
            "buyer_electronic_address": OrderXmlExtractor.get_text(
                buyer_party, "./cac:EndpointID", ns=ns
            ),
            "buyer_scheme_id": (
                buyer_party.find("./cac:EndpointID", ns).get("schemeID")
                if buyer_party.find("./cac:EndpointID", ns) is not None
                else None
            ),
            "buyer_country": (
                OrderXmlExtractor.get_text(
            ...
            "buyer_tax_scheme": OrderXmlExtractor.extract_party_tax_scheme(
                buyer_tax_scheme_elem, ns
            ),
        }
        ...
        seller = {
            "seller_account": OrderXmlExtractor.get_text(
                seller_supplier,
                "./cbc:CustomerAssignedAccountID",
                required=True,
            ...
            "seller_name": OrderXmlExtractor.get_text(
                seller_party,
                ".//cac:PartyName/cbc:Name",
                required=True,
            ...
            "seller_contact": OrderXmlExtractor.extract_contact(
                seller_contact_elem, ns
            ),
            "seller_tax_scheme": OrderXmlExtractor.extract_party_tax_scheme(
                seller_tax_scheme_elem, ns
            ),
        }
        ...
        payment_terms = (
            OrderXmlExtractor.get_text(
                transaction_conditions, "./cbc:Description", ns=ns
            )
        ...
        for elem in root.findall("./cac:OrderLine", ns):
            ...
            if line_item_elem is None:
                continue
            line_id = (
                OrderXmlExtractor.get_text(line_item_elem, "./cbc:ID", ns=ns) or ""
            )
            quantity_str = OrderXmlExtractor.get_text(
                line_item_elem, "./cbc:Quantity", ns=ns
            )
            ...
            if price_elem is None:
                raise HTTPException(
                    status_code=400, detail="Missing Price element in OrderLine"
                )
            price_amount_str = OrderXmlExtractor.get_text(
                price_elem, "./cbc:PriceAmount", ns=ns
            )
            base_quantity_str = OrderXmlExtractor.get_text(
                price_elem, "./cbc:BaseQuantity", ns=ns
            )
            price_amount = float(price_amount_str)
            base_quantity = float(base_quantity_str)
            if base_quantity == 0:
                raise HTTPException(
                    status_code=400, detail="BaseQuantity cannot be zero in OrderLine"
                )
            ...
            if item_elem is None:
                continue
            item_description = (
                OrderXmlExtractor.get_text(item_elem, "./cbc:Description", ns=ns) or ""
            )
