...
from src.exceptions.auth_exceptions import InvalidCredentialsError, MissingTokenError
from src.services.v1.auth_service import (
    authenticate_user,
    get_JWT,
    remove_JWT,
    token_logout_valid,
...
@router.post("/login", tags=["User Authentication (v1) - deprecated"])
...
@router.get("/validate/login", response_model=dict, tags=["User Token Validation (v1) - deprecated"])
...
@router.post("/logout", tags=["User Authentication (v1) - deprecated"])
...
@router.get("/validate/logout", response_model=dict,tags=["User Token Validation (v1) - deprecated"])
